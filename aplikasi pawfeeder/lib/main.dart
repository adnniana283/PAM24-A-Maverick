...
class _DiscussionScreenState extends State<DiscussionScreen> {
  ...
  // Function untuk mendapatkan diskusi dan balasan
  Future<void> getDiskusi() async {
    try {
      ...
      if (response.statusCode == 200) {
        setState(() {
          diskusi = json
              .decode(response.body); // Parsing seluruh topik beserta balasan
          isLoading = false;
          // Initialize controller untuk setiap topik
          _balasanControllers.clear();
  ...
  // Function untuk posting balasan
  Future<void> postBalasan(
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode({
          'id_topik': idTopik,
          'id_parent_balasan': null, // Bisa disesuaikan dengan balasan lain
      ...
      if (response.statusCode == 201) {
        ...
        // Setelah balasan disimpan, refresh diskusi
        getDiskusi();
      } else {
  ...
  void initState() {
    super.initState();
    getDiskusi(); // Fetch diskusi saat pertama kali layar dimuat
  }
    ...
    return Scaffold(
                      ...
                      children: [
                        ...
                        ListTile(
                          ...
                          title: TextField(
                            ...
                            onSubmitted: (value) {
                              if (value.isNotEmpty) {
                                postBalasan(item['id_topik'], value,
                                    1); // 1 adalah ID pengguna
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () async {
          ...
          if (shouldRefresh) {
            getDiskusi(); // Refresh diskusi jika ada diskusi baru yang diposting
          }
        },
        backgroundColor: Colors.pink,
...
class _ArtikelScreenState extends State<ArtikelScreen> {
  ...
  Future<void> getArtikel() async {
    try {
      ...
      if (response.statusCode == 200) {
        setState(() {
          artikel = json.decode(response.body); // Fetching the list of articles
          isLoading = false;
        });
      } else {
        setState(() {
  ...
  void initState() {
    super.initState();
    getArtikel(); // Fetch articles when screen loads
  }
                            ...
                            return Padding(
                              ...
                              child: Row(
                                children: [
                                  ...
                                  ClipRRect(
                                    borderRadius: BorderRadius.circular(10),
                                    child: article['foto_artikel'] != null
                                        ? Image.network(
