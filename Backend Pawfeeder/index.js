...
app.post("/pengguna", (req, res) => {
  ...
  if (email && password_hash && !username && !nama_lengkap) {
    const query =
      "SELECT * FROM Pengguna WHERE email = ? AND password_hash = ?";
...
app.post(
  ...
  (req, res) => {
    ...
    const location =
      tipe_kucing === "adopsi" ? lokasi_penampungan || null : null;
    const description = tipe_kucing === "adopsi" ? deskripsi || null : null;
    const contact =
      tipe_kucing === "adopsi" ? kontak_penampungan || null : null;
    ...
    connection.execute(
      ...
      (err, results) => {
        ...
        res
          .status(201)
          .json({ message: "Kucing berhasil ditambahkan", data: results });
      }
    );
  }
);
...
app.get("/riwayat-makan", (req, res) => {
  const query =
    "SELECT id_riwayat, id_kucing, CONVERT_TZ(waktu_makan, '+00:00', '+07:00') AS waktu_makan, jumlah_pemberian FROM RiwayatPemberianMakan";
  connection.query(query, (err, results) => {
    if (err) {
      return res.status(500).json({ error: err.message });
    }
...
app.post(
  ...
  (req, res) => {
    ...
    const { id_kucing, waktu_makan, berapa_kali_makan, kebutuhan_kalori } =
      req.body;

    // Hitung jumlah makanan per pemberian (dalam gram)
    const foodPerFeeding = kebutuhan_kalori
      ? kebutuhan_kalori / berapa_kali_makan
      : 100; // Default 100 gram per pemberian jika kebutuhan kalori tidak ada
    ...
    Promise.all(insertPromises)
      .then((results) => {
        ...
        mqttClient.publish(
          "feeder/jadwal",
          JSON.stringify(feedingSchedule),
          (err) => {
            if (err) {
              console.error("Error sending schedule to ESP32:", err);
              return res
                .status(500)
                .json({ error: "Failed to send schedule to feeder." });
            }
      ...
      .catch((err) => {
        ...
        res
          .status(500)
          .json({ error: "Failed to add schedules or history.", details: err });
      });
  }
);
...
app.post(
  ...
  [
    ...
    body("waktu_balasan")
      .isISO8601()
      .withMessage("Invalid ISO8601 format for waktu_balasan"),
  ...
  (req, res) => {
    ...
    const query =
      "INSERT INTO Balasan (id_topik, id_parent_balasan, konten_balasan, waktu_balasan, id_pengguna) VALUES (?, ?, ?, ?, ?)";
    connection.query(
      query,
      [id_topik, id_parent_balasan, konten_balasan, waktu_balasan, id_pengguna],
      (err, results) => {
        if (err) {
    ...
    mqttClient.publish(
      ...
      (err) => {
        if (err) {
          ...
          return res
            .status(500)
            .json({ error: "Failed to send message to feeder" });
        }
        console.log(`Sent to ESP32: ${status}`);
        res
          .status(200)
          .json({ message: `Perintah ${status} berhasil dikirim ke feeder.` });
      }
    );
  } else {
    res
      .status(400)
      .send('Perintah tidak valid, hanya "ON" atau "OFF" yang diterima.');
  }
});
